describe('Unit tests for loggers', () => {
  ...
  beforeEach(inject((_$log_, _logger_) => {
    $log = _$log_;
    logger = _logger_;
  }));
  ...
  it('Error log', (done) => {
    ...
    expect(
      JSON.stringify($log.error.logs[0]),
      JSON.stringify([`Error: ${status}`, err])
    );
    done();
  });
  ...
  it('Info log', (done) => {
    ...
    expect(
      JSON.stringify($log.info.logs[0]),
      JSON.stringify([`Info: ${status}`, data])
    );
    done();
  });
  ...
  it('Warning log', (done) => {
    ...
    expect(
      JSON.stringify($log.warn.logs[0]),
      JSON.stringify([`Warning: ${status}`, data])
    );
    done();
  });
});
